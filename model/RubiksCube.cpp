 ...
 */
string RubiksCube::getMove(MOVE idx) {
    switch (idx) {
        case MOVE::L:
            return "L";
        case MOVE::LPRIME:
 ...
 * Perform a move operation on using a Move index.
 */
RubiksCube &RubiksCube::move(MOVE idx) {
    switch (idx) {
        case MOVE::L:
            return this->l();
        case MOVE::LPRIME:
 ...
 * Invert a move.
 */
RubiksCube &RubiksCube::invert(MOVE idx) {
    switch (idx) {
        case MOVE::L:
            return this->lPrime();
        case MOVE::LPRIME:
...
void RubiksCube::print() const {
    ...
    // print Middle layer: FACES( left -> front -> right -> back )
    for (int row=0; row<=2; row++) {
        for (int col=0; col<=2; col++) {
...
string RubiksCube::getCornerColorString(uint8_t idx) const {
    string str = "";
    ...
    switch (idx) {
        ...
        case 1:
            ...
            break;
        // UBL
        case 2: // TODO: case 2 and case 3 need to be interchanged:
            ...
            break;
        // UBR
        case 3: // TODO: case 2 and case 3 need to be interchanged:
...
uint8_t RubiksCube::getCornerIndex(uint8_t idx) const {
